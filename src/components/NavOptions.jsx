      ...
      <div className="w-full h-full flex">
        {navOptions.map((item, idx) => (
            <div key={item.id} className="w-1/4 h-full relative group">
                ...
                <div className="absolute z-[2] w-full h-full bg-[rgba(254,210,221,0.6)] group-hover:opacity-0 transition-all duration-200"></div>
                <div className="absolute z-[3] left-0 right-0 mx-auto max-w-[80%] h-full flex flex-col items-center justify-center gap-y-6 text-white">
                    <FontAwesomeIcon icon={item.icon} className="text-[2.3vw]"/>
                    <p className="font-semibold tracking-wide text-[1.2vw]">{item.name}</p>
                    ...
                    <button className="bg-white w-fit h-fit p-3 hover-action deep-shadow">
                        <p className="font-black text-black text-[1vw]">LEARN MORE</p>
                    </button>
                </div>
            </div>
        ))}
